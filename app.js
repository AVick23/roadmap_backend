// app.js — Backend Roadmap 2026
import { topics, categories } from './data.js';

// ═══ STATE ═══
const state = {
    read: loadFromStorage('backendMapRead', {}),
    favorites: loadFromStorage('backendMapFav', []),
    filter: 'all',
    levelFilter: null,
    categoryFilter: null,
    searchQuery: ''
...
function renderDrawer() {
    ...
    ` + categories.map(cat => `
        <button class="nav-item ${state.categoryFilter === cat.id ? 'active' : ''}" data-category="${cat.id}">
...
function renderBottomNav() {
    ...
    nav.innerHTML = mainCategories.map(cat => `
        <button class="bottom-nav-item ${state.categoryFilter === cat.id ? 'active' : ''}" 
                data-category="${cat.id}" aria-label="${cat.title}">
    ...
    `).join('') + `
        <button class="bottom-nav-item ${state.categoryFilter === 'favorites' ? 'active' : ''}" 
                data-category="favorites" aria-label="Избранное">
            <span class="nav-indicator"></span>
...
function applyFilters() {
    ...
    cards.forEach(card => {
        ...
        if (state.filter === 'favorites') matchesFilter = state.favorites.includes(id);
        else if (state.filter === 'unread') matchesFilter = !state.read[id];
        
        let matchesCategory = true;
        if (state.categoryFilter === 'favorites') matchesCategory = state.favorites.includes(id);
...
function renderActiveFilters() {
    ...
    `).join('') + `
        <button class="active-chip" id="resetAllBtn" style="color: var(--md-sys-color-primary);">
            <span class="material-symbols-outlined" style="font-size: 16px;">refresh</span>Сбросить
        </button>
    `;
}
...
document.addEventListener('touchend', (e) => {
    ...
    if (edgeTouchStartX < 20 && endX - edgeTouchStartX > 100) openDrawer();
}, { passive: true });
...
document.getElementById('topicsContainer').addEventListener('click', (e) => {
    ...
    if (checkbox) {
        ...
        saveToStorage('backendMapRead', state.read);
        updateStats();
        updateDrawerStats();
    ...
    if (favBtn) {
        ...
        saveToStorage('backendMapFav', state.favorites);
        renderBottomNav();
...
document.getElementById('activeFilters').addEventListener('click', (e) => {
    ...
    if (e.target.closest('#resetAllBtn')) resetAllFilters();
});
...
document.getElementById('drawerNav').addEventListener('click', (e) => {
    ...
    if (navItem) {
        ...
        state.categoryFilter = (cat === 'all' || state.categoryFilter === cat) ? null : cat;
        applyFilters();
        renderDrawer();
        renderBottomNav();
...
document.getElementById('bottomNav').addEventListener('click', (e) => {
    ...
    if (navItem) {
        ...
        if (cat === 'favorites') {
            ...
            document.querySelectorAll('.chip[data-filter]').forEach(c => c.classList.remove('selected'));
            document.querySelector(`.chip[data-filter="${state.filter}"]`).classList.add('selected');
        } else {
...
document.querySelectorAll('.chip').forEach(chip => {
    chip.addEventListener('click', () => {
        ...
        if (level) {
            ...
            state.filter = filter;
            chip.classList.add('selected');
            if (filter === 'favorites') state.categoryFilter = 'favorites';
            else if (state.categoryFilter === 'favorites') state.categoryFilter = null;
        }
        applyFilters();
        renderDrawer();
        renderBottomNav();
...
function applyTheme(theme) {
    ...
    saveToStorage('backendMapTheme', theme);
}

let currentTheme = loadFromStorage('backendMapTheme', 'dark');
...
window.addEventListener('scroll', () => {
    ...
    document.getElementById('topAppBar').classList.toggle('scrolled', window.scrollY > 10);
}, { passive: true });
...
document.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'm') { e.preventDefault(); toggleDrawer(); }
    if (e.key === 'Escape') {
